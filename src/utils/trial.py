...
def main(argv, verbose=True):
    bedfile = argv[1]
    exon_db = {}
    for n, (gene_id, G, total, transcripts) in enumerate(
                                make_graph(bedfile, exon_db), start=1):

        '''G is a directed graph.'''
        if verbose:
            print >> sys.stderr, '%s, total nodes = %d' % (gene_id,
                                                        len(G.nodes()))
        ...
        if total > len(paths):
            ...
            if verbose:
                print >> sys.stderr, \
                    '\tTotal original path = %d, minimal path = %d' %\
                                                    (total, len(paths))
        else:
            for trns in transcripts:
                print '\t'.join(trns) # print original transcripts
            if verbose:
                print >> sys.stderr, '\tTotal original path = %d' % total
